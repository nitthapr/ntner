...
class TextDataSet(object):
    ...
    def create_feature(self, row):
        ...
        bert_tokens2 = ["[CLS]"] + bert_tokens + ["[SEP]"]
        ...
        for i in range(self.config["max_sequence_length"]):
            ...
            if ( i < len (bert_tokens2) and (bert_tokens2[i] == '[CLS]' or bert_tokens2[i] == '[SEP]')):
              word_seq = [self.char2idx.get("pad")] *  self.config["max_char_length"]
            # -- incase normal word --
            else:
              for j in range(self.config["max_char_length"]):
                  try:
                      if(bert_tokens2[i][j] in self.char2idx):
                          word_seq.append(self.char2idx.get(bert_tokens2[i][j]))
                      else:
