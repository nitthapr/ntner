...
class TextDataSet(object):
    ...
    def createCharOneHot(self, char_ids, char2idx):
        onehot = []
        for ch in char_ids:
            oh = self._to_one_hot(torch.LongTensor(ch),len(char2idx))
            reoh = oh.reshape(len(oh)*len(char2idx))
            onehot.append(reoh.tolist())
        return onehot
    ...
    def create_feature(self, row):
        ...
        #---- Start prapas convert char_ids to one-hot------
        char_ids = self.createCharOneHot(char_ids, self.char2idx)
