...
class InputFeature(object):
    ...
    def __init__(
            ...
            tokens, tok_map,
            # pos
            bert_poses, pos_ids, poses, #add by prapas
            # Cls data
        ...
        data[4]: list, pos ids #add by prapas
        """
        self.data = []
        # Bert data
        ...
        # pos data
        self.bert_poses = bert_poses
        self.pos_ids = pos_ids
        self.data.append(pos_ids)
        # Classification data
        self.cls = cls
        self.id_cls = id_cls
...
class TextDataSet(object):
    ...
    @staticmethod
    def create_vocabs(
            df, tokenizer, idx2labels_path, idx2pos_path, markup="IO",
            idx2cls_path=None, pad_idx=0, is_cls=False, idx2labels=None, idx2cls=None, idx2pos=None):
        ...
        if idx2pos is None:
            pos2idx = {"[PAD]": pad_idx, '[CLS]': 1, '[SEP]': 2, "X": 3}
            idx2ps = ["[PAD]", '[CLS]', '[SEP]', "X"]
        else:
        ...
        for _, row in tqdm(df.iterrows(), total=len(df), leave=False, desc="Creating pos vocabs"):
            poses = row.pos.split()
            origin_tokens = row.text.split()
            if is_cls and row.cls not in cls2idx:
                cls2idx[row.cls] = len(cls2idx)
                idx2cls.append(row.cls)
            for origin_token, pos in zip(origin_tokens, poses):
                cur_tokens = tokenizer.tokenize(origin_token)
                bert_poses = [pos] * (len(cur_tokens))
                for pos_ in bert_poses:
                    if pos_ not in pos2idx:
                        pos2idx[pos_] = len(pos2idx)
                        idx2ps.append(pos_)

        with open(idx2pos_path, "w", encoding="utf-8") as f:
            for pos in idx2ps:
                f.write("{}\n".format(pos))
        ...
        return label2idx, idx2label, cls2idx, idx2cls, pos2idx, idx2ps
    ...
    def createPosOneHot(self, pos_ids, pos2idx):
        onehot = []
        for pos in pos_ids:
            oh = self._to_one_hot(torch.LongTensor(pos),len(pos2idx))
            #reoh = oh.reshape(len(oh)*len(pos2idx))
            onehot.append(oh.tolist())
        return onehot
    ...
    def create_feature(self, row):
        ...
        origin_poses = row.pos.split()
        tok_map = []
        prev_label = ""
        for orig_token, label, pos in zip(orig_tokens, origin_labels, origin_poses):
        ...
        pos_ids = [self.pos2idx.get(l) for l in bert_poses]
        #pos_ids = self.createPosOneHot(pos_ids, self.pos2idx)
        ...
        while len(input_ids) < self.config["max_sequence_length"]:
            ...
            pos_ids.append(self.config["pad_idx"])
            input_mask.append(0)
            tok_map.append(-1)
        ...
        return InputFeature(
            ...
            bert_poses=bert_poses, pos_ids=pos_ids, poses=origin_poses,
            # Origin data
            tokens=orig_tokens,
            tok_map=tok_map,
            # Cls
