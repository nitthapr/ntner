...
def validate_step(dl, model, id2label, sup_labels, id2cls=None):
    model.eval()
    test_loss = 0 #add by prapas to store loss in this step
    idx = 0
    ...
    for batch in tqdm(dl, total=len(dl), leave=False):
        ...
        preds, loss = model.forward(batch) #edit by prapas add loss to tuple
        ...
        test_loss += loss.data.cpu().tolist() #add by prapas to store loss in this step
    ...
    if id2cls is not None:
        ...
        return clf_report, clf_report_cls, test_loss / idx #edit by prapas add test_loss to return function
    return clf_report, test_loss / idx #edit by prapas add test_loss to return function
...
class NerLearner(object):
    ...
    def fit_one_cycle(self, epoch, target_metric="f1"):
        train_loss = train_step(self.data.train_dl, self.model, self.optimizer, epoch) #edit by prapass to store epoch loss
        self.train_losses.append(train_loss) #add by prapasto store epoch
        if epoch % self.validate_every == 0:
            if self.data.train_ds.is_cls:
                #edit by prapas add loss in tuple to recieve return loss from validate step
                rep, rep_cls, test_loss = validate_step(
                ...
                #edit by prapas add loss in tuple to recieve return loss from validate step
                rep, test_loss = validate_step(
                    self.data.valid_dl, self.model, self.data.train_ds.idx2label, self.sup_labels)
            self.history.append(rep)
            self.test_losses.append(test_loss) #add by prapas to keep test loss in array
