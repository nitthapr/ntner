...
class BERTBaseClassifier(BERTNerModel):
    ...
    def score(self, batch):
        #input_, labels_mask, input_type_ids, not_use, cls_ids = batch #mod to fix error add not_use to unpack empty labels
        input_embeddings = self.embeddings(batch)
        return self.clf.score(input_embeddings, batch[-1]) #batch[-1] = cls_ids
...
class BERTBiLSTMAttnClassifier(BERTNerModel):
    ...
    def score(self, batch):
        ...
        output, _ = self.lstm.forward(input_embeddings, labels_mask)
        output, _ = self.attn(output, output, output, None)
        return self.clf.score(output, batch[-1]) #data[-1] = last tuple, that is clf
