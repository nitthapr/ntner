...
class BERTCharBiLSTMCRF(BERTNerModel):
    ...
    def create(cls,
               ...
               num_char_dict=399,
               # BiLSTM params
               embedding_size=768+64, hidden_dim=512, rnn_layers=1, lstm_dropout=0.5,
...
class BERTCharBiLSTMCRF2(BERTNerModel):
    ...
    def create(cls,
               ...
               embedding_size=768+64, hidden_dim=512, rnn_layers=1, lstm_dropout=0.5,
               # BiLSTM params for char encoders
               char_lstm_embedding_size=32, char_lstm_hidden_dim=64, char_lstm_rnn_layers=1, char_lstm_dropout=0.5,
               # CRFDecoder params
               crf_dropout=0.5,
               # Global params
               device="cuda"):
        embeddings = BERTEmbedder.create(model_name=model_name, device=device, mode=mode, is_freeze=is_freeze)
        charencodings = CHAREncoder2.create(num_char_dict=num_char_dict, embedding_size=char_lstm_embedding_size,hidden_dim=char_lstm_hidden_dim,rnn_layers=char_lstm_rnn_layers,lstm_dropout=char_lstm_dropout,device=device, mode=mode, is_freeze=is_freeze)
        lstm = BiLSTM.create(
                embedding_size=embedding_size, hidden_dim=hidden_dim, rnn_layers=rnn_layers, dropout=lstm_dropout)
        crf = CRFDecoder.create(label_size, hidden_dim, crf_dropout)
        return cls(embeddings, charencodings, lstm, crf, device)
